...
Definition f_main := {|
  ...
  fn_params := nil;
  fn_vars := nil;
  fn_temps := ((_b, tbool) :: (_n, tint) :: (_m, tint) :: (_res, tint) ::
               nil);
  fn_body :=
(Ssequence
  (Ssequence
    (Sset _b (Ecast (Econst_int (Int.repr 1) tint) tbool))
    (Ssequence
      (Sset _n (Econst_int (Int.repr 2) tint))
      (Ssequence
        (Sset _m (Econst_int (Int.repr 3) tint))
        (Ssequence
          (Sifthenelse (Etempvar _b tbool)
            (Sset _res (Etempvar _n tint))
            (Sset _res (Etempvar _m tint)))
