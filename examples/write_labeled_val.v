...
Definition _high : ident := 54%positive.
Definition _highptr : ident := 51%positive.
Definition _low : ident := 55%positive.
Definition _lowptr : ident := 52%positive.
Definition _main : ident := 56%positive.
Definition _v : ident := 49%positive.
Definition _write_labeled_val : ident := 53%positive.

Definition f_write_labeled_val := {|
  fn_return := tvoid;
  fn_callconv := cc_default;
  fn_params := ((_v, tint) :: (_b, tbool) :: (_highptr, (tptr tint)) ::
                (_lowptr, (tptr tint)) :: nil);
  fn_vars := nil;
  fn_temps := nil;
  fn_body :=
(Sifthenelse (Etempvar _b tbool)
  (Sassign (Ederef (Etempvar _highptr (tptr tint)) tint) (Etempvar _v tint))
  (Sassign (Ederef (Etempvar _lowptr (tptr tint)) tint) (Etempvar _v tint)))
|}.
...
Definition f_main := {|
  ...
  fn_callconv := cc_default;
  fn_params := nil;
  fn_vars := ((_high, tint) :: (_low, tint) :: nil);
  fn_temps := nil;
  fn_body :=
(Ssequence
  (Ssequence
    (Scall None
      (Evar _write_labeled_val (Tfunction
                                 (Tcons tint
                                   (Tcons tbool
                                     (Tcons (tptr tint)
      ...
      ((Econst_int (Int.repr 1) tint) :: (Econst_int (Int.repr 1) tint) ::
       (Eaddrof (Evar _high tint) (tptr tint)) ::
       (Eaddrof (Evar _low tint) (tptr tint)) :: nil))
    (Ssequence
      (Scall None
        (Evar _write_labeled_val (Tfunction
                                   (Tcons tint
                                     (Tcons tbool
                                       (Tcons (tptr tint)
                                         (Tcons (tptr tint) Tnil)))) tvoid
                                   cc_default))
        ((Econst_int (Int.repr 2) tint) :: (Econst_int (Int.repr 0) tint) ::
         (Eaddrof (Evar _high tint) (tptr tint)) ::
         (Eaddrof (Evar _low tint) (tptr tint)) :: nil))
      (Sreturn (Some (Ebinop Oadd (Evar _high tint) (Evar _low tint) tint)))))
...
((___builtin_fabs,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_write_labeled_val, Gfun(Internal f_write_labeled_val)) ::
 (_main, Gfun(Internal f_main)) :: nil);
prog_public :=
(_main :: _write_labeled_val :: ___builtin_debug :: ___builtin_nop ::
