...
Definition _cl : ident := 57%positive.
Definition _four : ident := 56%positive.
Definition _i : ident := 53%positive.
Definition _isPublic : ident := 50%positive.
Definition _main : ident := 59%positive.
Definition _n : ident := 51%positive.
Definition _s : ident := 58%positive.
Definition _sum : ident := 52%positive.
Definition _sum_cl_array : ident := 55%positive.
Definition _v : ident := 54%positive.
Definition _t'1 : ident := 60%positive.

Definition f_sum_cl_array := {|
  fn_return := tint;
  fn_callconv := cc_default;
  fn_params := ((_a, (tptr tint)) :: (_isPublic, (tptr tbool)) ::
                (_n, tint) :: nil);
  fn_vars := nil;
  fn_temps := ((_sum, tint) :: (_i, tint) :: (_v, tint) :: nil);
  fn_body :=
(Ssequence
      ...
      (Sloop
        (Ssequence
          (Sifthenelse (Ebinop Olt (Etempvar _i tint) (Etempvar _n tint)
            ...
            Sbreak)
          (Sifthenelse (Ederef
                         (Ebinop Oadd (Etempvar _isPublic (tptr tbool))
                           (Etempvar _i tint) (tptr tbool)) tbool)
            (Ssequence
              (Sset _v
                (Ederef
                  (Ebinop Oadd (Etempvar _a (tptr tint)) (Etempvar _i tint)
                    (tptr tint)) tint))
              (Sset _sum
                (Ebinop Oadd (Etempvar _sum tint) (Etempvar _v tint) tint)))
            Sskip))
        (Sset _i
          (Ebinop Oadd (Etempvar _i tint) (Econst_int (Int.repr 1) tint)
            tint))))
    (Sreturn (Some (Etempvar _sum tint)))))
|}.
...
Definition v_cl := {|
  gvar_info := (tarray tbool 4);
  gvar_init := (Init_int8 (Int.repr 1) :: Init_int8 (Int.repr 0) ::
...
(Ssequence
  (Ssequence
    (Ssequence
      (Scall (Some _t'1)
        (Evar _sum_cl_array (Tfunction
                              (Tcons (tptr tint)
                                (Tcons (tptr tbool) (Tcons tint Tnil))) tint
                              cc_default))
        ((Evar _four (tarray tint 4)) :: (Evar _cl (tarray tbool 4)) ::
...
((___builtin_fabs,
 ...
 (___builtin_debug,
   Gfun(External (EF_external "__builtin_debug"
     ...
     {|cc_vararg:=true; cc_unproto:=false; cc_structret:=false|})) ::
 (_sum_cl_array, Gfun(Internal f_sum_cl_array)) :: (_four, Gvar v_four) ::
 (_cl, Gvar v_cl) :: (_main, Gfun(Internal f_main)) :: nil);
prog_public :=
(_main :: _cl :: _four :: _sum_cl_array :: ___builtin_debug ::
